import 'package:flutter/material.dart';


class AppTheme {
  AppTheme._();



  static const Color yellowColor=Color(0xFFFECC49);
  static const Color bgColor=Color(0xFF3B3B3D);
  static const Color red=Color(0xFFE34343);
  static const Color addNewTextFieldBorder=Color(0xFFCECECE);
  static const Color addNewTextFieldFocusBorder=Color(0xFF4E4E4E);
  static  Color addNewTextFieldText=Color(0xFF434343);

  static  Color grey=Color(0xFF787878);


  static  TextStyle hintText=TextStyle(fontFamily: 'RR',fontSize: 20,color: addNewTextFieldText.withOpacity(0.5));
  static TextStyle TSWhite20=TextStyle(fontFamily: 'RR',fontSize: 20,color: Colors.white,letterSpacing: 0.1);
  static TextStyle TSWhite16=TextStyle(fontFamily: 'RR',fontSize: 18,color: Colors.white,letterSpacing: 0.1);


}
